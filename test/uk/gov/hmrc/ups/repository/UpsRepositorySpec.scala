...
class UpsRepositorySpec
  ...
  "TTL index" should {
    "be configured for updatedAt field" in {
      val indexes = repository.collection.listIndexes().toFuture().futureValue
      val ttlIndex = indexes.find(_.get("name").exists(_.asString().getValue == "updatedAtTtlIdx"))

      ttlIndex mustBe defined
      ttlIndex.get.get("expireAfterSeconds").map(_.asNumber().longValue()) mustBe Some(30 * 24 * 60 * 60L)
    }
  }
