...
class ExclusiveScheduledJobSpec extends AnyWordSpec with Matchers with ScalaFutures {
  ...
  class SimpleJob extends ExclusiveScheduledJob {
    ...
    override val name = "simpleJob"

    override lazy val initialDelay: FiniteDuration = FiniteDuration(1, TimeUnit.MINUTES)

    override lazy val interval: FiniteDuration = FiniteDuration(1, TimeUnit.MINUTES)
